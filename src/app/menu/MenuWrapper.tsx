...
export default function MenuWrapper({ categories, skipSplash = false }: { categories: Category[]; skipSplash?: boolean }) {
  ...
  // Colori via inline styles per garantire funzionamento
  const dark = isEvening;
  const colors = dark
    ? { bg: '#1e293b', text: '#fff', textSec: 'rgba(255,255,255,0.92)', textTer: 'rgba(255,255,255,0.85)', border: 'rgba(255,255,255,0.3)', logo: '#fff' }
    : { bg: '#f9fafb', text: '#111827', textSec: '#374151', textTer: '#4b5563', border: '#e5e7eb', logo: '#1e293b' };

  return (
    <>
      <main
        className={`min-h-screen py-8 px-4 pb-10 transition-all duration-1000 ease-out overflow-x-hidden ${
      ...
      <div className="max-w-2xl mx-auto flex flex-col min-h-screen">
        <header className="flex flex-col items-center mt-6 mb-10">
          <div
            className="h-28 w-56 opacity-95 [mask-size:contain] [mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-size:contain] [-webkit-mask-repeat:no-repeat] [-webkit-mask-position:center] transition-all duration-500 hover:opacity-100 hover:scale-105 animate-fade-in-down"
            style={{ maskImage: 'url(/torri-dellacqua-logo.svg)', WebkitMaskImage: 'url(/torri-dellacqua-logo.svg)', backgroundColor: colors.logo }}
            role="img"
            aria-label="Torri dell'Acqua"
          />
          <h1 className="text-2xl font-light mt-6 tracking-[0.2em] uppercase transition-all duration-500 hover:tracking-[0.25em] animate-fade-in" style={{ fontFamily: 'ui-serif, Georgia, serif', animationDelay: '0.2s', color: colors.text }}>Menu</h1>
          <div className="h-px my-4 animate-expand-width" style={{ animationDelay: '0.4s', backgroundColor: colors.border, width: '4rem' }} />
          <p className="text-xs tracking-wide font-light animate-fade-in" style={{ fontFamily: 'ui-serif, Georgia, serif', animationDelay: '0.6s', color: colors.textSec }}>{MENU_NOTE}</p>
          <Link
            href={BOOKING_URL}
            className="mt-4 text-xs font-light tracking-wide transition-colors animate-fade-in"
            style={{ fontFamily: 'ui-serif, Georgia, serif', animationDelay: '0.5s', color: colors.textTer }}
          >
            Prenota un tavolo
          </Link>
        </header>
        ...
        <footer className="mt-16 pt-10 border-t animate-fade-in-up" style={{ animationDelay: '0.8s', borderColor: colors.border }}>
          <div className="flex flex-col items-center text-center">
            <div
              className="h-20 w-40 opacity-85 [mask-size:contain] [mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-size:contain] [-webkit-mask-repeat:no-repeat] [-webkit-mask-position:center] mb-6 transition-all duration-500 hover:opacity-100 hover:scale-105"
              style={{ maskImage: 'url(/torri-dellacqua-logo.svg)', WebkitMaskImage: 'url(/torri-dellacqua-logo.svg)', backgroundColor: colors.logo }}
              role="img"
              aria-label="Torri dell'Acqua"
            />
            <address className="not-italic text-sm space-y-2.5 mb-6 font-light" style={{ fontFamily: 'ui-serif, Georgia, serif', color: colors.textSec }}>
              <a
              ...
              <div className="flex justify-center gap-4 text-xs">
                <a href={`tel:${RESTAURANT.phone.replace(/\s/g, '')}`} className="transition-all duration-300 hover:scale-105 inline-block">
                  {RESTAURANT.phone}
                </a>
                <span style={{ color: colors.textTer }}>·</span>
                <a href={`mailto:${RESTAURANT.email}`} className="transition-all duration-300 hover:scale-105 inline-block">
                  {RESTAURANT.email}
                </a>
              </div>
            </address>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group inline-flex items-center gap-2.5 px-6 py-3 rounded-full border text-sm font-light transition-all duration-500 hover:scale-105 hover:shadow-lg"
              style={{ fontFamily: 'ui-serif, Georgia, serif', borderColor: colors.border, color: colors.text }}
            >
          ...
          {/* Credits */}
          <div className="mt-8 pt-6 border-t animate-fade-in" style={{ animationDelay: '1s', borderColor: colors.border }}>
