...
export default function MenuSplash({ onEnter }: { onEnter: () => void }) {
  ...
  return (
    <div className={`fixed inset-0 z-40 flex items-center justify-center transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      {/* Content */}
      <div className="relative z-10 flex flex-col items-center px-8 max-w-lg w-full animate-fade-in-up">
        {/* Logo with elegant fade */}
        <div
          className={`h-36 w-72 ${isEvening ? 'bg-white' : 'bg-[#2a2a2a]'} [mask-size:contain] [mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-size:contain] [-webkit-mask-repeat:no-repeat] [-webkit-mask-position:center] mb-16 opacity-95 transition-all duration-700 hover:opacity-100 hover:scale-105`}
          style={{
        ...
        <button
          onClick={onEnter}
          className={`${theme.button} ${theme.buttonRing} px-12 py-4 rounded-full text-base font-light tracking-[0.15em] uppercase transition-all duration-300 transform hover:scale-105 hover:-translate-y-0.5 active:scale-100 active:translate-y-0 mb-8 focus:outline-none focus:ring-4`}
        ...
        <a
          ...
          className={`inline-flex items-center gap-2.5 px-6 py-3 rounded-full border ${theme.instagramBorder} text-sm font-light tracking-wide transition-all duration-300 hover:scale-105`}
          style={{ fontFamily: 'ui-serif, Georgia, serif' }}
        >
