...
export default async function MenuPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#63B1D2] to-[#4a9ec4] text-white py-5 px-4 pb-8">
      <div className="max-w-xl mx-auto flex flex-col min-h-screen">
        <header className="flex flex-col items-center mt-4 mb-6">
          ...
          />
          <h1 className="text-xl font-semibold text-white mt-3">Menu</h1>
          <p className="text-white/80 text-xs mt-1">{MENU_NOTE}</p>
        </header>
        ...
        <footer className="mt-12 pt-8 border-t border-white/20">
          <div className="flex flex-col items-center text-center">
            <div
              className="h-16 w-36 bg-white/90 [mask-size:contain] [mask-repeat:no-repeat] [mask-position:center] [-webkit-mask-size:contain] [-webkit-mask-repeat:no-repeat] [-webkit-mask-position:center] mb-4"
              style={{ maskImage: 'url(/torri-dellacqua-logo.svg)', WebkitMaskImage: 'url(/torri-dellacqua-logo.svg)' }}
              role="img"
              aria-label="Torri dell'Acqua"
            />
            <address className="not-italic text-white/90 text-sm space-y-1.5 mb-4">
              <a
              ...
              <div className="flex justify-center gap-3 text-xs">
                <a href={`tel:${RESTAURANT.phone.replace(/\s/g, '')}`} className="hover:text-white transition-colors">
                  {RESTAURANT.phone}
                </a>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/40 text-white text-sm hover:bg-white/10 hover:border-white transition-all"
            >
