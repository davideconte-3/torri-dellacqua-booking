...
export default function BookingSplash({ onEnter, isExiting = false }: { onEnter: () => void; isExiting?: boolean }) {
  ...
  const theme = isEvening
    ? {
        ...
        linkBorder: 'border-white/50 text-white hover:bg-white/20 hover:border-white/80',
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 flex flex-col items-center px-8 max-w-lg w-full transition-all ease-out ${
        ...
        <div className={`flex flex-wrap justify-center gap-3 mb-8 ${isExiting ? 'opacity-0 translate-y-6' : ''}`}>
          <Link
            href="/menu?skipSplash=1"
        ...
        <a
          ...
          className={`inline-flex items-center gap-2.5 px-6 py-3 rounded-full border ${theme.linkBorder} text-sm font-light tracking-wide transition-all duration-300 ${
